...
parser rules (Choice _ rs ls) =
  let rs' = (parser rules . (M.!) rules <$> rs)
      ls' = (parser rules . (M.!) rules <$> ls)
   in try (mconcat rs') <|> mconcat ls'
...
solution1 :: Input -> Int
solution1 input =
  let (bareRules, messages) = parseOrDie inputParser input
      rules :: Rules
      rules = indexRules bareRules
   in length . filter id $ isValid rules <$> messages
...
solution2 input =
  let (bareRules, messages) = parseOrDie inputParser input
      rules :: Rules
      rules = indexRules bareRules
   in length . nub $ [message | message <- messages, r0 <- rule0s, isValid (M.insert 0 r0 rules) message]

solution :: Solution
solution = PureSolution solution1 142 solution2 294
