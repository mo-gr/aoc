...
solution1 :: Input -> Int
solution1 input =
  let is = parseOrDie inputParser input
      two_neighbours = fst <$> filter (\(_, xs) -> length xs == 2) ((\i -> (i, findMatchingNeighbours is i)) <$> is)
   in product (imageId <$> two_neighbours)
...
matchesSeaMonster ps x y =
  if isBlack ps (x + 18) (y + 0)
    && isBlack ps (x + 0) (y + 1)
    && isBlack ps (x + 5) (y + 1)
    && isBlack ps (x + 6) (y + 1)
    && isBlack ps (x + 11) (y + 1)
...
solution2 input =
  let is = parseOrDie inputParser input
      fieldAndNeighbours = (\i -> (i, findMatchingNeighbours is i)) <$> is
      aCorner = case rotateImage . rotateImage . rotateImage . fst <$> find (\(_i, is') -> length is' == 2) fieldAndNeighbours of
        Just x -> x
      ...
      firstCol = findNeighbours is DBottom aCorner
      assembled = findNeighbours is DRight <$> firstCol
      fused = Image 0 96 $ fuseImages assembled
      monsterCount = length $ detectMonster 96 fused
   in --  putStrLn $ pretty 96 $ pixels fused
      countWaves (pixels fused) - (monsterCount * monsterSize)

solution :: Solution
solution = PureSolution solution1 2297 solution2 1957
