...
visible w N (x,y) = let c' = (x, y-1) in case getAt w c' of
   Just Floor -> visible w N c'
   v          -> v
visible w NE (x,y) =let c' = (x+1, y-1) in case getAt w c' of
   Just Floor -> visible w NE c'
   v          -> v
visible w E (x,y) = let c' = (x+1, y) in case getAt w c' of
   Just Floor -> visible w E c'
   v          -> v
visible w SE (x,y) = let c' = (x+1, y+1) in case getAt w c' of
   Just Floor -> visible w SE c'
   v          -> v
visible w S (x,y) = let c' = (x, y+1) in case getAt w c' of
   Just Floor -> visible w S c'
   v          -> v
visible w SW (x,y) = let c' = (x-1, y+1) in case getAt w c' of
   Just Floor -> visible w SW c'
   v          -> v
visible w W (x,y) = let c' = (x-1, y) in case getAt w c' of
   Just Floor -> visible w W c'
   v          -> v
visible w NW (x,y) = let c' = (x-1, y-1) in case getAt w c' of
...
runTilStable tf w = let w' = tickWorld tf w in
  if w == w' then w else runTilStable tf w'
...
solution1 :: IO Int
solution1 = do
  world <- fromRight [[]] <$> parseFromFile inputParser "AOC11.input"
  return . countOccupied . runTilStable tick $ world

-- 2072
solution2 :: IO Int
solution2 = do
  world <- fromRight [[]] <$> parseFromFile inputParser "AOC11.input"
  return . countOccupied . runTilStable visibleTick $ world
