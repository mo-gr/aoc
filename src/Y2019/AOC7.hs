    ...
    [WritingOutput, _, _, _, _] ->
      modify
        ( \case
            (m0 : m1 : ms)
              -> m0 {runState = Running}
                   : m1 {runState = Running, input = input m1 ++ [output m0]} : ms
      ...
      modify
        ( \case
            (m0 : m1 : m2 : ms)
              -> m0
                   : m1 {runState = Running}
                       : m2 {runState = Running, input = input m2 ++ [output m1]} : ms
            _ -> error "unexpected state"
        )
        >> runThrusterUntilHalt
        ...
        ( \case
            (m0 : m1 : m2 : m3 : ms)
              -> m0
                   : m1
                       : m2 {runState = Running}
                           : m3 {runState = Running, input = input m3 ++ [output m2]} : ms
            _ -> error "unexpected state"
        )
        >> runThrusterUntilHalt
            ...
            (m0 : m1 : m2 : m3 : m4 : ms)
              -> m0
                   : m1
                       : m2
                           : m3 {runState = Running}
                               : m4 {runState = Running, input = input m4 ++ [output m3]} : ms
            _ -> error "unexpected state"
        )
        >> runThrusterUntilHalt
        ...
        ( \case
            (m0 : m1 : m2 : m3 : m4 : ms)
              -> m0 {runState = Running, input = input m0 ++ [output m4]}
                   : m1 : m2 : m3 : m4 {runState = Running} : ms
            _ -> error "unexpected state"
        )
        >> runThrusterUntilHalt
...
prop_example_add = H.withTests 1 $
  H.property $ do
...
prop_example_read = H.withTests 1 $
  H.property $ do
...
prop_example = H.property $ do
  ...
  let m' = execState runUntilHalt m
  m'
    H.=== (buildMachine [30, 1, 1, 4, 2, 5, 6, 0, 99])
      { opCode = 8,
        runState = Halt
      }
