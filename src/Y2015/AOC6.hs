...
import Text.Parsec (try, string, newline, many1)
import Data.Functor (($>))
...
eval lights o = foldl (f o) lights (pointsInOp o)
  where f (TurnOn _) ls l = M.insert l On ls
...
evalDim lights o = foldl (f o) lights (pointsInOp o)
  where f (TurnOn _) ls l = M.insertWith (dim 1) l (Dim 1) ls
